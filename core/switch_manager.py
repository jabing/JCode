    ...
    def get(self, level: str, key: str = None) -> Any:
        ...
        if level == "global":
            ...
            if key:
                if key == "enabled":
                    return config.get("enabled", True)
                raise KeyError(f"Invalid global key: {key}")
            return config.get("enabled", True)
    ...
    def is_forced_enable(self, file_path: Optional[str] = None, operation: Optional[str] = None) -> bool:
        ...
        if file_path:
            ...
            for pattern in file_patterns:
                ...
                # Handle **/config/** patterns specifically
                # This pattern means: any path containing /config/ anywhere
                if normalized_pattern == '**/config/**':
                    if '/config/' in normalized_path or normalized_path.startswith('config/'):
                        return True
                # Handle **/prefix patterns (match anywhere)
                elif normalized_pattern.startswith('**/'):
                    suffix = normalized_pattern[3:]  # Remove **/
                    # Replace * with [^/]* for proper matching
                    suffix = suffix.replace('*', '[^/]*')
                    # Match if any part of path matches this pattern
                    try:
                        import re as _re
                        if _re.search(suffix, normalized_path):
                            return True
                    except:
                        pass
                # Handle patterns ending with /** (match this prefix and any subdirs)
                elif normalized_pattern.endswith('/**'):
                    prefix = normalized_pattern[:-3]  # Remove /**
                    # Match if path starts with this prefix
        ...
        if file_path:
            ...
            for pattern in file_patterns:
                ...
                # Handle patterns starting with **/
                    ...
                    suffix = normalized_pattern[3:]  # Remove **/
                    # Match if path ends with this suffix after any directories
                    if normalized_path.endswith(suffix) or f'/{suffix}' in normalized_path:
                        return True
        ...
        if file_path:
            ...
            for pattern in file_patterns:
                ...
                # Path.match() supports ** patterns correctly
                try:
                    if Path(normalized_path).match(normalized_pattern):
                        return True
                except (ValueError, TypeError):
                    # Fall back to fnmatch if Path.match() fails
                    if fnmatch.fnmatch(normalized_path, normalized_pattern):
                        return True
        ...
        if file_path:
            ...
            for pattern in file_patterns:
                ...
                # Convert glob patterns to regex (order matters!)
                regex_pattern = normalized_pattern
                
                # First, escape special regex characters (except glob wildcards)
                # Then process glob patterns in the correct order
                
                # Convert ** to a marker, process later
                regex_pattern = regex_pattern.replace('**', '\x00')
                
                # Escape regex special characters
                regex_pattern = re.escape(regex_pattern)
                
                # Now convert the marker back to .*
                regex_pattern = regex_pattern.replace('\x00', '.*')
                
                # Convert * to [^/]* (any characters except /)
                ...
                try:
                    ...
                    # If regex fails, fall back to simple fnmatch
        ...
        if file_path:
            ...
            for pattern in file_patterns:
                ...
                # Convert **/ to (?:.*/)? (any directories or none)
                regex_pattern = regex_pattern.replace('**/', '(?:.*/)?')
                
                # Convert /** to /(?:.*) (any directories after)
                regex_pattern = regex_pattern.replace('/**', '/(?:.*)')
        ...
        if file_path:
            ...
            for pattern in file_patterns:
                normalized_pattern = pattern.replace('\\', '/')
                # Handle ** patterns by converting to fnmatch-compatible patterns
